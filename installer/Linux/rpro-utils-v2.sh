...
SERVICE_LOCATION=/lib/systemd/system/red5pro.service
IS_64_BIT=0
OS_NAME=
OS_VERSION=
MODE=0
...
install_jsvc_rhl()
{
	write_log "Installing jsvc on rhle"
...
download_from_url()
{
	...
	lecho "Downloading Red5pro from url"
...
auto_install_rpro_url()
{
	...
	echo "Preparing to install Red5Pro from $RED5PRO_DOWNLOAD_URL"
	sleep 2
	download_from_url

	if [ "$latest_rpro_download_success" -eq 0 ]; then
		echo "Failed to download Red5pro distribution from $RED5PRO_DOWNLOAD_URL. Please contact support!"
		pause
	fi
...
#######################################################
	...
	touch $SERVICE_LOCATION

	# write script to file
	echo "$service_script" > "$SERVICE_LOCATION"
...
start_red5pro_service()
{
	...
	if [ ! -f "$SERVICE_LOCATION" ];	then
		...
		lecho "Red5Pro service was found at $SERVICE_LOCATION"
...
#	echo "1. CHECK EXISTING RED5PRO INSTALLATION"
	...
	echo "2. INSTALL RED5PRO FROM URL"
...
prerequisites_jsvc()
{
	...
	# Checking wget
	lecho "Checking for service manager"
	sleep 2
